...
def make_table(df):
    ...
    table += "\\toprule \n"
    table += " & \\multicolumn{3}{c}{Violation at $\\alpha=10\\%$}  & \\multicolumn{3}{c}{Violation at $\\alpha=5\\%$} \\\\ \n"
    table += "\\cmidrule(r){2-4}  \\cmidrule(r){5-7} \n"
    table += "Model & APS & RAPS & LAC & APS & RAPS & LAC \\\\ \n"
    table += "\\midrule \n"
    ...
    for model in df.Model.unique():
        ...
        table += str(np.round(dfmodel[dfmodel.alpha == 0.1]["LAC violation"].item(),3)) + " & "
        ...
        table += str(np.round(dfmodel[dfmodel.alpha == 0.05]["LAC violation"].item(),3)) + " " 
        table += "\\\\ \n"
    
    table += "\\bottomrule \n"
    table += "\\end{tabular} \n"
    table += "\\caption{\\textbf{Adaptiveness results after automatically tuning $\\lambda$.} We report the median worst-stratum coverage violations of \\aps\\, \\raps\\, and LAC over 10 trials. See Appendix~\\ref{app:optimizing-adaptiveness} for experimental details.} \n"
    table += "\\label{table:LAC-tunelambda} \n"
    table += "\\end{table} \n"
    return table    
...
def get_worst_violation(model, logits, alpha, strata, randomized, n_data_conf, n_data_val, pct_paramtune, bsz):
    ### Data Loading
    ...
    # Conformalize the model with LAC 
    conformal_model = ConformalModelLogits(model, calib_loader, alpha=alpha, allow_zero_sets=True, naive=False, LAC=True)
    LAC_worst_violation = get_violation(conformal_model, val_loader, strata, alpha)

    return aps_worst_violation, raps_worst_violation, LAC_worst_violation
...
def experiment(modelname, datasetname, datasetpath, num_trials, alpha, strata, randomized, n_data_conf, n_data_val, pct_paramtune, bsz):
    ...
    LAC_violations = np.zeros((num_trials,))
    for i in tqdm(range(num_trials)):
        aps_violations[i], raps_violations[i], LAC_violations[i] = get_worst_violation(model, logits, alpha, strata, randomized, n_data_conf, n_data_val, pct_paramtune, bsz)
        print(f'\n\tAPS Violation: {np.median(aps_violations[0:i+1]):.3f}, RAPS Violation: {np.median(raps_violations[0:i+1]):.3f}, LAC Violation: {np.median(LAC_violations[0:i+1]):.3f}\033[F', end='')
    print('')
    return np.median(aps_violations), np.median(raps_violations), np.median(LAC_violations)
...
if __name__ == "__main__":
    ...
    cache_fname = "./.cache/LAC_tune_lambda_df.csv"

    strata = [[0,1],[2,3],[4,6],[7,10],[11,100],[101,1000]]
    try:
        df = pd.read_csv(cache_fname)
    except:
        df = pd.DataFrame(columns=['Model','alpha','APS violation','RAPS violation', 'LAC violation'])
        for i in range(m):
            modelname, alpha = params[i]
            print(f'Model: {modelname} | Desired coverage: {1-alpha}')

            APS_violation_median, RAPS_violation_median, LAC_violation_median = experiment(modelname, datasetname, datasetpath, num_trials, alpha, strata, randomized, n_data_conf, n_data_val, pct_paramtune, bsz) 
            df = df.append({"Model": modelname,
                            "alpha": alpha,
                            "APS violation": APS_violation_median,
                            "RAPS violation": RAPS_violation_median,
                            "LAC violation": LAC_violation_median}, ignore_index=True) 
        df.to_csv(cache_fname)

    table_str = make_table(df)
    table = open("outputs/LAC_tunelambda.tex", 'w')
